#rice bowl
execute unless score @s jmmf.data matches 1 store success score @s jmmf.data if score @s jmmf.count matches 1 if data storage jmmf:cooking_station {input:{ingredients:[{components:{"minecraft:custom_data":{joshmats:{tag:["rice"]}}}, count:2}]}} run function jmmf:block/cooking_station/cooking/recipes/rice_bowl/check
