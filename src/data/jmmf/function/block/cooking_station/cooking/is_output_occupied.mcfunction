...
execute store result score jmmf:temp1 jmmf.count.output run data get entity @s equipment.offhand.components."minecraft:max_stack_size"
execute if items entity @s weapon.offhand *[max_stack_size=64] run scoreboard players set jmmf:temp1 jmmf.count.output 64
...
#  - Fail if there is enough space
scoreboard players operation jmmf:temp1 jmmf.count.output -= @s jmmf.count.output

# tellraw @a ["free output: ",{type:"score",score:{name:"jmmf:temp1",objective:"jmmf.count.output"}}]
# tellraw @a ["craft count: ",{type:"score",score:{name:"@s",objective:"jmmf.craft_count"}}]
execute if score @s jmmf.craft_count <= jmmf:temp1 jmmf.count.output run return fail
